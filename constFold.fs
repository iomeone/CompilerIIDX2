...
let findi x env = (match Map.find x env with Int(i) -> i | _ -> raise (System.Collections.Generic.KeyNotFoundException ""))
let findf x env = (match Map.find x env with Float(d) -> d | _ -> raise (System.Collections.Generic.KeyNotFoundException ""))
let findt x env = (match Map.find x env with Tuple(ys) -> ys | _ -> raise (System.Collections.Generic.KeyNotFoundException ""))
