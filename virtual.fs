#indent "off"
module Virtual
(* translation into PowerPC assembly (infinite number of virtual registers) *)

open Asm

let data = ref [] (* 浮動小数点数の定数テーブル *)

let classify xts ini addf addi =
  List.fold
    (fun acc (x, t) -> match t with
       | Type.Unit -> acc
       | Type.Float -> addf acc x
       | _ -> addi acc x t) ini xts

let separate xts = 
  classify 
    xts 
    ([], []) 
    (fun (int, float) x -> (int, float @ [x]))
    (fun (int, float) x _ -> (int @ [x], float))

let expand xts ini addf addi = 
  classify
    xts
    ini
    (fun (offset, acc) x -> (offset + 1, addf x offset acc))
    (fun (offset, acc) x t -> (offset + 1, addi x t offset acc))

let rec g env = function (* 式の仮想マシンコード生成 *)
  | Closure.Unit -> Ans (Nop)
  | Closure.Int (i) -> Ans (Li (i))
  | Closure.Float (d) -> 
		let i = Emit.getftoi d in Ans(FLi i)
  | Closure.Neg (x) -> Ans (Neg (x))
  | Closure.Add (x, y) -> Ans (Add (x, V (y)))
  | Closure.Sub (x, y) -> Ans (Sub (x, V (y)))
  | Closure.Mul (x, y) -> Ans (Mul (x, V (y)))
  | Closure.Div (x, y) -> Ans (Div (x, V (y)))
  | Closure.FNeg (x) -> Ans (FNeg (x))
  | Closure.FAdd (x, y) -> Ans (FAdd (x, y))
  | Closure.FSub (x, y) -> Ans (FSub (x, y))
  | Closure.FMul (x, y) -> Ans (FMul (x, y))
  | Closure.FDiv (x, y) -> Ans (FDiv (x, y))
  | Closure.IfEq (x, y, e1, e2) -> 
      (match Map.find x env with
			 | Type.Bool | Type.Int -> Ans (IfEq (x, V (y), g env e1, g env e2))
			 | Type.Float -> Ans (IfFEq (x, y, g env e1, g env e2))
			 | _ -> failwith "equality supported only for bool, int, and float")
  | Closure.IfLE (x, y, e1, e2) ->
      (match Map.find x env with
			 | Type.Bool | Type.Int -> Ans (IfLE (x, V (y), g env e1, g env e2))
			 | Type.Float -> Ans (IfFLE (x, y, g env e1, g env e2))
			 | _ -> failwith "inequality supported only for bool, int, and float")
  | Closure.Let ((x, t1), e1, e2) ->
      let e1' = g env e1 in
      let e2' = g (Map.add x t1 env) e2 in
				concat e1' (x, t1) e2'
  | Closure.Var (x) ->
      (match Map.find x env with
			 | Type.Unit -> Ans (Nop)
			 | Type.Float -> Ans (FMr (x))
			 | _ -> Ans (Mr (x)))
  | Closure.MakeCls ((x, t), {Closure.entry = l; Closure.actual_fv = ys}, e2) ->
      (* closure のアドレスをセットしてからストア *)
      let e2' = g (Map.add x t env) e2 in
      let (offset, store_fv) = 
				expand
					(List.map (fun y -> (y, Map.find y env)) ys)
					(1, e2')
					(fun y offset store_fv -> seq (Stfd (y, x, C (offset)), store_fv))
					(fun y _ offset store_fv -> seq (Stw (y, x, C (offset)), store_fv)) in
						Let ((x, t), Mr (reg_hp), 
								 Let ((reg_hp, Type.Int), Add (reg_hp, C (offset)), 
								 let z = Id.genid "l" in  
									 Let ((z, Type.Int), SetL(l), 
										 seq (Stw (z, x, C (0)), store_fv))))
  | Closure.AppCls (x, ys) ->
      let (int, float) = separate (List.map (fun y -> (y, Map.find y env)) ys) in
				Ans (CallCls (x, int, float))
  | Closure.AppDir (Id.L(x), ys) ->
      let (int, float) = separate (List.map (fun y -> (y, Map.find y env)) ys) in
				Ans (CallDir (Id.L(x), int, float))
  | Closure.Tuple (xs) -> (* 組の生成 *)
      let y = Id.genid "t" in
      let (offset, store) = 
				expand
					(List.map (fun x -> (x, Map.find x env)) xs)
					(0, Ans (Mr (y)))
					(fun x offset store -> seq (Stfd (x, y, C (offset)), store))
					(fun x _ offset store -> seq (Stw (x, y, C (offset)), store))  in
						Let ((y, Type.Tuple (List.map (fun x -> Map.find x env) xs)), Mr (reg_hp),
								 Let ((reg_hp, Type.Int), Add (reg_hp, C (offset)), store))
  | Closure.LetTuple (xts, y, e2) ->
      let s = Closure.fv e2 in
      let (offset, load) = 
				expand
					xts
					(0, g (M.add_list xts env) e2)
					(fun x offset load ->
						 if not (Set.contains x s) then load 
						 else fletd (x, Lfd (y, C (offset)), load))
					(fun x t offset load ->
						 if not (Set.contains x s) then load 
						 else Let ((x, t), Lwz (y, C (offset)), load)) in
	load
  | Closure.Get (x, y) -> (* 配列の読み出し *)
      let offset = Id.genid "o" in  
			(match Map.find x env with
				 | Type.Array (Type.Unit) -> Ans (Nop)
				 | Type.Array (Type.Float) ->
						 Let ((offset, Type.Int), Slw (y, C (0)), 
							Ans (Lfd (x, V (offset))))
				 | Type.Array (_) ->
						 Let ((offset, Type.Int), Slw (y, C (0)),(*配列参照のオフセットはここ(シフト量)*)
							Ans (Lwz (x, V (offset))))
				 | _ -> failwith "virtual.fs id:1")
  | Closure.Put (x, y, z) ->
      let offset = Id.genid "o" in 
			(match Map.find x env with
				 | Type.Array (Type.Unit) -> Ans (Nop)
				 | Type.Array (Type.Float) -> 
						 Let ((offset, Type.Int), Slw (y, C (0)),
						Ans (Stfd (z, x, V (offset)))) 
				 | Type.Array (_) ->
						 Let ((offset, Type.Int), Slw (y, C (0)), 
						Ans (Stw (z, x, V (offset)))) 
				 | _ -> failwith "virtual.fs id:2")
  | Closure.ExtArray (Id.L(x)) -> Ans(SetL(Id.L("min_caml_" ^ x)))
  | Closure.Print (x) -> Ans (Print (x))
  | Closure.PrintShort (x) -> Ans (PrintShort (x))
  | Closure.Scan (x) -> Ans (Scan (x))
	| Closure.Sqrt (x) -> Ans (Sqrt (x))

(* 関数の仮想マシンコード生成 *)
let h { Closure.name = (Id.L(x), t); Closure.args = yts; 
	Closure.formal_fv = zts; Closure.body = e} =
  let (int, float) = separate yts in
  let (offset, load) = 
    expand
      zts
      (1, g (Map.add x t (M.add_list yts (M.add_list zts Map.empty))) e)
      (fun z offset load -> fletd (z, Lfd (reg_cl, C (offset)), load))
      (fun z t offset load -> Let ((z, t), Lwz (reg_cl, C (offset)), load)) in
    match t with
      | Type.Fun (_, t2) ->
	  { name = Id.L(x); args = int; fargs = float; body = load; ret = t2 }
      | _ -> failwith "virtual.fs id:3"

(* プログラム全体の仮想マシンコード生成 *)
let f (Closure.Prog (fundefs, e)) =
  data := [];
  let fundefs = List.map h fundefs in
  let e = g Map.empty e in
    Prog (!data, fundefs, e)
